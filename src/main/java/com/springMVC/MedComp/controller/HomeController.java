...
public class HomeController {
	...
	public ModelAndView signInValidate(@RequestParam Map<String,String> reqParam){
		...
		ArrayList<Userinfo> results =(ArrayList) session.createQuery(hql)
	...
	public ModelAndView signUpValidatePage(@RequestParam Map<String,String> reqParam){
		...
		if( !(firstName.matches("[A-Za-z]*")) || !(lastName.matches("[A-Za-z]*")) || (middleName!=null && !middleName.matches("[A-Za-z]*")) ){
	...
	public ModelAndView diseasePredictor(HttpServletResponse response) throws IOException{
		...
		resultList= session.createQuery(hql)
						.getResultList();
		session.getTransaction().commit();
		session.close();
	...
	public ModelAndView showSymptoms(@RequestParam Map<String,String> reqParam) throws IOException{
		...
		List<Symptom> resultList=session.createQuery(hql)
						.setParameter("part", bodyPart)
						.getResultList();
	...
	public ModelAndView showAilment(@RequestParam Map<String,String> reqParam) throws IOException{
		...
		List<Symptom> resultList=session.createQuery(hql0)
						.setParameter("part", bodyPart)
						.getResultList();
		String hql = "SELECT symId FROM Symptom WHERE symName IN (:names) ";
		List<Integer> symIdList=session.createQuery(hql)
						.setParameter("names", symNames)
						.getResultList();
		//System.out.println(symIdList);
		String hql2 = "SELECT disId FROM Mapping WHERE symId IN (:ids) ORDER BY disId";
		List<Integer> disIdList= session.createQuery(hql2)
				.setParameter("ids", symIdList)
				.getResultList();
		...
		List<String> nameList= session.createQuery(hql3)
